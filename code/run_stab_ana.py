...
test = True
if test:
    s.run(["python3", "stability_analysis.py", "--data", "aifb", "--criterion", "cosine-knn", "--dim_red", "no" ])
else:
    for data in ("aifb", "bgs", "mutag"):
        for nlp_model in ("cbow", "skip-gram"):
            for dim_red in ("pca2d", "pca3d", "tsne2d", "tsne3d"):
                ...
                s.run(["python3", "stability_analysis.py", "--data", data, "--nlp_model", nlp_model, "--dim_red", dim_red,
    ...
    for data in ("aifb", "bgs", "mutag"):
        for nlp_model in ("cbow", "skip-gram"):
            for criterion in ("cosine-knn", "aligned_cosine", "sec_order_cosine"):
                ...
                s.run(["python3", "stability_analysis.py", "--data", data, "--nlp_model", nlp_model, "--dim_red", "no",
                       "--criterion", criterion])